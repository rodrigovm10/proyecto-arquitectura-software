...
function LoginBdt() {
    ...
    async function getData() {
      await Auth.currentAuthenticatedUser()
        .then(async data => {
          ...
          const sub = DataStore.observeQuery(BDT, c => c.correo.eq(data.attributes.email), { limit: 1 }).subscribe(({ items }) => {
            setUserData(items[0])
          })
          return () => {
            sub.unsubscribe()
  ...
  return (
    <div>
      Loading...
      {session ? nombreGrupo === 'trabajador' ? <Navigate to='/registro-bdt' /> : <Navigate to='/login-empresa' /> : <></>}
    </div>
  )
}
