...
export async function correoBdtPostulacion(vacante, bdt) {
  try {
    console.log("sendBajaLogica: Inicio de la función");
    // Obtén el token de autenticación del usuario actual
    const user = await Auth.currentSession();
    ...
    // URL de la función lambda que envía el correo electrónico
    const url = process.env.REACT_APP_API_REDLABORAL + "/enviar-correo-postulacion-bdt";
...
export async function correoEmpresaPostulacion(vacante, bdt) {
  try {
    console.log("sendBajaLogica: Inicio de la función");
    // Obtén el token de autenticación del usuario actual
    const user = await Auth.currentSession();
    ...
    // URL de la función lambda que envía el correo electrónico
    const url = process.env.REACT_APP_API_REDLABORAL + "/enviar-correo-postulacion-empresa";
    
    // Datos que coinciden con la estructura de 'event' de la función lambda
    const requestData = {
      empresa: {
        nombreEmpresa:vacante.nombreEmpresa,
        email: vacante.emailEmpresa,
        nombreVacante: vacante.nombre,
        nombre: bdt.nombre,
        cv: bdt.pdfImagenUrl
      },
