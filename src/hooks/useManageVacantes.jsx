		...
		try {
			...
			if (emailEmpresa) {
				...
				newVacantes = await DataStore.query(
					Vacante,
					c => c.and(c => [c.visible.eq(true), c.numeroPlazas.gt(0), c.emailEmpresa.eq(emailEmpresa)]),
					{
		...
		try {
			...
			if (municipio !== 'Todos' && area === 'Todas' && salarioMin === 0 && salarioMax === 100000) {
				let newVacantes = await DataStore.query(
					Vacante,
					c => c.and(c => [c.visible.eq(true), c.numeroPlazas.gt(0), c.municipio.eq(municipio)]),
					{
						sort: s => s.createdAt(SortDirection.DESCENDING)
					}
				)
			...
			if (area !== 'Todas' && municipio === 'Todos' && salarioMin === 0 && salarioMax === 100000) {
				newVacantes = await DataStore.query(
					Vacante,
					c => c.and(c => [c.visible.eq(true), c.numeroPlazas.gt(0), c.area.eq(area)]),
					{
						sort: s => s.createdAt(SortDirection.DESCENDING)
					}
				)
			...
			if (salarioMin !== 0 && area === 'Todas' && municipio === 'Todos' && salarioMax === 100000) {
				newVacantes = await DataStore.query(
					Vacante,
					c => c.and(c => [c.visible.eq(true), c.numeroPlazas.gt(0), c.salarioMin.ge(salarioMin)]),
					{
						sort: s => s.createdAt(SortDirection.DESCENDING)
					}
				)
			...
			if (salarioMax !== 100000 && area === 'Todas' && municipio === 'Todos' && salarioMin === 0) {
				newVacantes = await DataStore.query(
					Vacante,
					c => c.and(c => [c.visible.eq(true), c.numeroPlazas.gt(0), c.salarioMax.le(salarioMax)]),
					{
						sort: s => s.createdAt(SortDirection.DESCENDING)
					}
				)
			...
			if (municipio !== 'Todos' && area !== 'Todas' && salarioMin === 0 && salarioMax === 100000) {
				newVacantes = await DataStore.query(
					Vacante,
					c => c.and(c => [c.visible.eq(true), c.numeroPlazas.gt(0), c.municipio.eq(municipio), c.area.eq(area)]),
					{
						sort: s => s.createdAt(SortDirection.DESCENDING)
					}
				)
			...
			if (municipio === 'Todos' && area !== 'Todas' && salarioMin !== 0 && salarioMax === 100000) {
				newVacantes = await DataStore.query(
					Vacante,
					c => c.and(c => [c.visible.eq(true), c.numeroPlazas.gt(0), c.area.eq(area), c.salarioMin.ge(salarioMin)]),
					{
						sort: s => s.createdAt(SortDirection.DESCENDING)
					}
				)
			...
			if (municipio === 'Todos' && area !== 'Todas' && salarioMin === 0 && salarioMax !== 100000) {
				newVacantes = await DataStore.query(
					Vacante,
					c => c.and(c => [c.visible.eq(true), c.numeroPlazas.gt(0), c.area.eq(area), c.salarioMax.le(salarioMax)]),
					{
						sort: s => s.createdAt(SortDirection.DESCENDING)
					}
				)
	...
	async function listVacantesNoVisibles({ emailEmpresa }) {
		try {
			const newVacantes = await DataStore.query(
				Vacante,
				c => c.and(c => [c.visible.eq(false), c.numeroPlazas.gt(0), c.emailEmpresa.eq(emailEmpresa)]),
				{
					sort: s => s.createdAt(SortDirection.DESCENDING)
				}
			)
