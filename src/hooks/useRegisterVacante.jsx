...
export function useRegisterVacante() {
  ...
  const handleInputChange = e => {
    ...
    if (name === 'prestaciones' || name === 'habilidadesTecnicas' || name === 'habilidadesBlandas' || name === 'idioma') {
  ...
  const handleClickSave = ({ name, value }) => {
    if (name === 'idiomaConNivel') value = stringToSave + '/' + secondStringToSave
  ...
  const validations = ({ name, value }) => {
    if (name === 'idiomaConNivel') {
      if (value === '/') {
        basicAlert({ title: 'Valor invalido', icon: 'info', text: 'No se pueden ingresar valores vacíos' })
        return true
      }
      if (!secondStringToSave) {
        basicAlert({ title: 'Valor invalido', icon: 'info', text: 'Debe de añadir el nivel del idioma' })
        return true
      }
      if (!stringToSave) {
        basicAlert({ title: 'Valor invalido', icon: 'info', text: 'Debe de añadir el idioma' })
        return true
      }
    }
    if (!value) {
    ...
    if (datosVacante[name].includes(value)) {
      basicAlert({ title: 'Valor repetido', icon: 'info', text: `El valor "${value}" ya se encuentra ingresado, ingresa uno diferente.` })
      return true
    }
  }
  ...
  return { datosVacante, errors, setDatosVacante, handleInputChange, handleClickSave, stringToSave, secondStringToSave, handleClickDelete, handleNumberChange, somePropertyIsNull }
}
